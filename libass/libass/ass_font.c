                       ...
                       uint32_t symbol, int *face_index, int *glyph_index)
{
    ...
    FT_Face face = 0;
    ass_msg(font->library, MSGL_INFO,
            "ass_font_get_index 0x%X "
            "font for (%s, %d, %d)", symbol, font->desc.family,
            font->desc.bold, font->desc.italic); //storm
    *glyph_index = 0;
                            ...
                            int index, ASS_Hinting hinting, int deco)
{
    ...
    printf("ass_font::ass_font_get_glyph ch: %u glyph ft_glyph_top: %d left: %d advance.x: %ld y: %ld \
            format: %d FT_GLYPH_FORMAT_OUTLINE: %d\n"
            , ch , ((FT_BitmapGlyph) glyph)->top, ((FT_BitmapGlyph) glyph)->left
            , glyph->advance.x, glyph->advance.y
            , face->glyph->format, FT_GLYPH_FORMAT_OUTLINE); //storm
