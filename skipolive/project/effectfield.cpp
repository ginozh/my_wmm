			...
			if (before_keyframe == after_keyframe) {
				...
				if (before_key.type == EFFECT_KEYFRAME_HOLD) {
					...
					if (before_key.type == EFFECT_KEYFRAME_BEZIER && after_key.type == EFFECT_KEYFRAME_BEZIER) {
						// cubic bezier
						double t = cubic_t_from_x(timecode*GLFRAMERATE, before_key.time, before_key.time+get_validated_keyframe_handle(before_keyframe, true), after_key.time+get_validated_keyframe_handle(after_keyframe, false), after_key.time);
						...
						double t = quad_t_from_x(timecode*GLFRAMERATE, before_key.time, before_key.time+get_validated_keyframe_handle(before_keyframe, true), after_key.time);
						value = quad_from_t(before_dbl, before_dbl+before_key.post_handle_y, after_dbl, t);
					} else {
						// this keyframe is the bezier one
						double t = quad_t_from_x(timecode*GLFRAMERATE, before_key.time, after_key.time+get_validated_keyframe_handle(after_keyframe, false), after_key.time);
