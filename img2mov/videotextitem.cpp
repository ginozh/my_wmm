...
void GraphicsTextItem::createAssInfo(float factor)
{
    if(m_globalTextAttr && m_globalTextAttr->m_iDurationText)
    {
        ...
        start.chop(1);
        QString end = QDateTime(QDate::currentDate()).addMSecs(m_globalTextAttr->m_iStartTimeText+m_globalTextAttr->m_iDurationText-20).toString("hh:mm:ss.zzz"); //-10:如上 uncomplete:临界
        end.chop(1);
