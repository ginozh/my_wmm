...
void GraphicsScene::setVideoTextAttr(Element* element, GlobalTextAttr* globalTextAttr)
{
    if(!globalTextAttr)
    {
        //uncomplete
...
#endif
QString GraphicsScene::createTotalAssInfo(QSize qSize/*=QSize(0,0)*/)
{
    if(qSize.width()<=0)
    {
        qSize=QSize(width(), height());
    }
    QString qsAss=QString("[Script Info]\n"
            "; Script generated by FFmpeg/Lavc57.24.102\n"
            "ScriptType: v4.00+\n"
            "PlayResX: %1\n"
            "PlayResY: %2\n"
            "\n"
            "[V4+ Styles]\n").arg(qSize.width()).arg(qSize.height());
    QString qsStyles(
    ...
    QString qsEvents;
    QMap<Element*, GraphicsTextItem*>::iterator it = m_mapText.begin();
    for(; it!=m_mapText.end(); it++)
    //for( auto key: m_mapText.keys() )
    {
        Element* element = it.key();
        GraphicsTextItem *tmptextItem = it.value(); //m_mapText.value( key );
        ...
        if(element && element->valid() && tmptextItem && tmptextItem->textAttr())
        {
            tmptextItem->createAssInfo(qSize.width()/width());
            if(!tmptextItem->textAttr()->m_qsStyle.isEmpty())
            {
...
#endif 
void GraphicsScene::displayVideoText(Element* element, bool isDisplay)
{
    if(m_mapText.contains(element))
    {
        GraphicsTextItem *textItem = m_mapText[element];
...
void GraphicsScene::activeVideoText(Element* element, const QString& oritxt)
{
    if(m_mapText.contains(element))
    {
