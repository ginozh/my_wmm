...
vec4 FUNCNAME(vec2 tc) {
	...
	float processScal =8.0* iGlobalTime*iGlobalTime*iGlobalTime; // scale amp (5x)^3.0
	
	float processColorSp = pow(cos(PI*(iGlobalTime-0.5)),2.0)*0.01;
	float processMB = processScal*0.2;// the blur trend is same as scale
	float rotaDegree=0.0;
	float processRota = PI/90.0*rotaDegree*iGlobalTime;//iGlobalTime..[0,0.5]
	
	//time > 0.5
	float tmp= 1.0-iGlobalTime;
	float processWarpR = 8.0* tmp*tmp*tmp;// warp amp (2(1.0-x))^3.0
	
	//float processScalR = 2.0-iGlobalTime*2.0; // scale amp
	//float processMBR = (1.0-iGlobalTime)*0.2;
	float processScalR = processWarpR;
	float processMBR= processWarpR*0.2;
	float processRotaR = PI/90.0*rotaDegree*(processWarpR*0.5);//iGlobalTime..[0.5,1.0]
	
	float shadingXL = (normalizedRandom(vec2(1.0,iGlobalTime))-0.5)*processWarp*0.07;
	float shadingyL = (normalizedRandom(vec2(10.0,iGlobalTime/3.0))-0.5)*processWarp*0.07;
	vec2 shadingVec2L = vec2(shadingXL,shadingyL);
	
	float shadingXR = (normalizedRandom(vec2(1.0,iGlobalTime))-0.5)*processWarpR*0.07;
	float shadingyR = (normalizedRandom(vec2(10.0,iGlobalTime/3.0))-0.5)*processWarpR*0.07;
