...
vec4 FUNCNAME(vec2 tc) {
	...
	//time > 0.5
	float processWarpR = (iGlobalTime-1.0)*(iGlobalTime-1.0)*(iGlobalTime-1.0)*(iGlobalTime-1.0);//pow((2.0*(1.0-iGlobalTime)),4.0);// warp amp
	...
	float processRotaR = PI/90.0*rotaDegree*processScalR*0.5;//iGlobalTime..[0.5,1.0]
