...
vec4 FUNCNAME(vec2 tc) {
    ...
    { 	
		...
		return color/float(Samples);
			...
			//rollDir = vec2( 0.99999*sin(2.3561944*(iGlobalTime-0.33333333))+1.0,0.0);
			vec4 color = vec4(0.0,0.0,0.0,1.0);
			vec2 amp = vec2(1.0,1.0/(1.0+2.0*(1.0-uv.x)*(1.0-rollDir.x/2.0)));
			uv = (uv - vec2(0.5))*amp.y+vec2(0.5);

			float bottom = 1.0;
			float top = 1.0/amp.y;
			float tan = ((top - bottom+0.0000001)/2.0)/1.0 ;
			float H = 0.5*bottom/tan;
