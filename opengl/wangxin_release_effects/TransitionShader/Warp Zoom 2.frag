...
{
	vec2 temp;
	temp.x = dot(vec2(cos(theta), -sin(theta)), uv - center) + center.x;
	temp.y = dot(vec2(sin(theta), cos(theta)), uv - center) + center.y;
	return(temp);
}
...
vec4 FUNCNAME(vec2 tc) {
	...
	float processScalR = 1.0-0.8*exp(-8.0*(iGlobalTime-0.2));
	float processMBR=0.5*(iGlobalTime-1.0)*(iGlobalTime-1.0);
