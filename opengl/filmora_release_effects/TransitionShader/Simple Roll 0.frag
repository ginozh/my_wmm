...
vec4 FUNCNAME(vec2 tc) {
    ...
    { 	
		...
		rollDir =  vec2(  1.0-pow(abs(sin(PI/2.0*(5.2*(iGlobalTime-1.0/5.2)))),0.5), 0.0);
		
		processMB = clamp(rollDir.x,-0.3,0.3);
		processColorSp = clamp((1.0+rollDir.x)*0.1,-0.01,0.01);
		...
		{
			rollDir = vec2( 75.0*iGlobalTime*iGlobalTime+30.0*iGlobalTime,0.0);
		...
		{
			rollDir = vec2( -abs( 0.2*sin(20.0*3.1415926*(iGlobalTime-0.4)/(11.1/3.0*iGlobalTime+0.33333333))*exp(-8.0*(iGlobalTime-0.4))),0.0);
			processMBR=clamp(rollDir.x,-0.01,0.01);
			processColorSp =0.2*clamp(rollDir.x,-0.01,0.01);
