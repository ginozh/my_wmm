...
vec4 FUNCNAME(vec2 tc) {
	...
	float processMBR=0.8*(iGlobalTime-1.0)*(iGlobalTime-1.0);
	
	
	
	if (iGlobalTime<0.3333333333333)
    { 	
		//float HoriScale = 9.0*iGlobalTime*iGlobalTime+3.0*iGlobalTime;
		float HoriScale = pow( (3.3333333333*iGlobalTime),7.0)*2.0;
		float vertiScale = 0.3*(1.0-uv.y)*HoriScale+1.0;

		vec4 color = vec4(0.0,0.0,0.0,1.0);
		//vec2 amp = vec2(1.0/(1.0+uv.y*3.3*boxFlit),1.0/(1.0+uv.y*0.3*boxFlit));
		vec2 amp = vec2(1.0/(1.0+(1.0-uv.y)*HoriScale),1.0/vertiScale);
		//vec2 amp = vec2(1.0,1.0);
		uv = scale(uv,center,amp)-vec2(0.0,HoriScale*0.1);
		
		float bottom = 1.0;
		float top = (1.0+uv.y*HoriScale);
		float tan = vertiScale/((top - bottom)/2.0+0.000001) ;
		float H = 0.5*bottom*tan;
		vec2 center =vec2(0.5,-H);
		...
		return color/float(Samples);
			...
			//rollDir = vec2( 0.0,0.99999*sin(2.3561944*(iGlobalTime-0.33333333)));
			float temp = 1.5*(iGlobalTime-0.3333333333333) - 1.0;
			rollDir = -vec2( 0.0,1.0-temp*temp*temp*temp)*2.0;
			vec4 color = vec4(0.0,0.0,0.0,1.0);
			vec2 amp = vec2(1.0/(1.0+2.0*(uv.y)*(1.0+rollDir.y/2.0)),1.0);
	
			uv = (uv - vec2(0.5))*amp.x+vec2(0.5);

			float bottom = 1.0/amp.x;
			float top = 1.0;
			vec2 center =vec2(0.5,1.0/(bottom-top+0.0000001)+1.0);
			dir = normalize(uv-center);
