...
vec4 FUNCNAME(vec2 tc) {
	...
	float rotateDeta = PI/180.0*180.0;
...
if (iGlobalTime<0.2)
    { 	
		uv = fishEye (uv,center,prop,50.0*fishEyeAmp);
		...
		uv = fishEye (uv,center,prop,90.0*fishEyeAmp);
		if(iGlobalTime<0.4)//0.3-0.6
		{
			
			
			rollDir = vec2( 0.0,-97.5*(iGlobalTime-0.4)*(iGlobalTime-0.4)-0.1);
			
			processMBR = 0.4-iGlobalTime;
		...
		{
			rollDir = vec2( 0.0,-0.1*exp(-10.0*(iGlobalTime-0.4)));
			processMBR =0.0;
