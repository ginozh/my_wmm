...
static void load_fonts_from_dir(ASS_Library *library, const char *dir)
{
    ...
    while (1) {
        ...
        void *data = NULL;//read_file(library, fullname, &bufsize); //storm
        if (data) {
        ...
        for (int x = 0; x < priv->n_font; x++) {
            ...
            if (score < score_min) {
                ...
                if (!check_glyph(font, code))
                    continue;
                printf("ass_fontselect.c::find_font code: %u fullname: %s score: %u score_min: %u match_font families: %s fullname: %s postscript_name: %s\n", code, fullname, score, score_min, *font->families, *font->fullnames, font->postscript_name);//storm
                score_min = score;
                selected = font;
            }
