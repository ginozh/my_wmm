...
class EditableItemDialog : public QDialog {
	...
	void BrowseClicked()
	{
		...
		QString path = QFileDialog::getOpenFileName(
			RecordGlobal::instance()->GetRecordDialog(), QTStr("Browse"), curPath,
			filter);
		if (path.isEmpty())
			return;
...
void WidgetInfo::EditListAddFiles()
{
	...
	QStringList files = QFileDialog::getOpenFileNames(
		RecordGlobal::instance()->GetRecordDialog(), title, QT_UTF8(default_path),
		QT_UTF8(filter));
...
void WidgetInfo::EditListEdit()
{
	...
	if (type == OBS_EDITABLE_LIST_TYPE_FILES) {
		...
		if (pathDir.exists())
			path = QFileDialog::getExistingDirectory(
				RecordGlobal::instance()->GetRecordDialog(), QTStr("Browse"),
				item->text(),
				QFileDialog::ShowDirsOnly |
					QFileDialog::DontResolveSymlinks);
		else
