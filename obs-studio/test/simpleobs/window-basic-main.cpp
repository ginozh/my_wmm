...
{
    RecordGlobal::instance()->SetRecordDialog(this);
	...
	if (ret > 0) {
		if (QFile::exists(styleSheetPath)) {
			...
			// RecordGlobal::instance()->setStyleSheet(path);
			setStyleSheet(path);  // tmp storm
		}
	}
	...
	bool sceneGrid = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
					 "gridMode");
...
#endif
	stringstream name;
	name << "OBS " << RecordGlobal::instance()->GetVersionString();
	...
	const char *geometry = config_get_string(RecordGlobal::instance()->GlobalConfig(),
				    ...
				    obs_data_array_t *savedProjectorList)
{
	...
	const char *sceneCollection = config_get_string(
		RecordGlobal::instance()->GlobalConfig(), "Basic", "SceneCollection");
...
{
#if 0
    // 去掉初始的audio, 否则会有2个audio source
	bool hasDesktopAudio = HasAudioDevices(RecordGlobal::instance()->OutputAudioSource());
	bool hasInputAudio = HasAudioDevices(RecordGlobal::instance()->InputAudioSource());

	if (hasDesktopAudio)
		ResetAudioDevice(RecordGlobal::instance()->OutputAudioSource(), "default",
				 Str("Basic.DesktopDevice1"), 1);
	if (hasInputAudio)
		ResetAudioDevice(RecordGlobal::instance()->InputAudioSource(), "default",
...
#if 0
	LoadAudioDevice(DESKTOP_AUDIO_1, 1, data); // storm 移除,否则可能有2个
...
retryScene:
	...
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "SceneCollection",
			  name);
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "SceneCollectionFile",
			  file_base.c_str());
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	bool oldResolutionDefaults = config_get_bool(
		RecordGlobal::instance()->GlobalConfig(), "General", "Pre19Defaults");
...
{
	bool oldEncDefaults = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "General",
					      "Pre23Defaults");
	bool useNV = false;// EncoderAvailable("ffmpeg_nvenc") && !oldEncDefaults; // storm
...
bool OBSBasic::InitBasicConfig()
{
	...
	if (config_get_string(basicConfig, "General", "Name") == nullptr) {
		const char *curName = config_get_string(RecordGlobal::instance()->GlobalConfig(),
							"Basic", "Profile");
...
void OBSBasic::OBSInit()
{
	...
	const char *sceneCollection = config_get_string(
		RecordGlobal::instance()->GlobalConfig(), "Basic", "SceneCollectionFile");
	char savePath[512];
	char fileName[512];
	int ret;
	...
	sceneDuplicationMode = config_get_bool(
		RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "SceneDuplicationMode");
	swapScenesMode = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
					 "SwapScenesMode");
	editPropertiesMode = config_get_bool(
		RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "EditPropertiesMode");

#define SET_VISIBILITY(name, control)                                         \
	do {                                                                  \
		if (config_has_user_value(RecordGlobal::instance()->GlobalConfig(),              \
					  "BasicWindow", name)) {             \
			bool visible = config_get_bool(RecordGlobal::instance()->GlobalConfig(), \
						       "BasicWindow", name);  \
...
#undef SET_VISIBILITY
	...
	previewEnabled = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
					 "PreviewEnabled");
	...
	bool alwaysOnTop = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
					   "AlwaysOnTop");
	...
	const char *dockStateStr = config_get_string(
		RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "DockState");
	if (!dockStateStr) {
	...
	bool pre23Defaults = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "General",
					     "Pre23Defaults");
	if (pre23Defaults) {
		bool resetDockLock23 = config_get_bool(
			RecordGlobal::instance()->GlobalConfig(), "General", "ResetDockLock23");
		if (!resetDockLock23) {
			config_set_bool(RecordGlobal::instance()->GlobalConfig(), "General",
					"ResetDockLock23", true);
			config_remove_value(RecordGlobal::instance()->GlobalConfig(),
					    "BasicWindow", "DocksLocked");
			config_save_safe(RecordGlobal::instance()->GlobalConfig(), "tmp", nullptr);
		}
	}

	bool docksLocked = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
					   "DocksLocked");
	...
	bool has_last_version = config_has_user_value(RecordGlobal::instance()->GlobalConfig(),
						      "General", "LastVersion");
	bool first_run =
		config_get_bool(RecordGlobal::instance()->GlobalConfig(), "General", "FirstRun");

	if (!first_run) {
		config_set_bool(RecordGlobal::instance()->GlobalConfig(), "General", "FirstRun",
				true);
		config_save_safe(RecordGlobal::instance()->GlobalConfig(), "tmp", nullptr);
	}
	...
	ToggleMixerLayout(config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
					  "VerticalVolControl"));
#if 0
	...
	if (!opt_studio_mode) {
		SetPreviewProgramMode(config_get_bool(RecordGlobal::instance()->GlobalConfig(),
						      "BasicWindow",
						      "PreviewProgramMode"));
	} else {
...
void OBSBasic::ReceivedIntroJson(const QString &text)
{
...
#if OBS_RELEASE_CANDIDATE > 0
	uint32_t lastVersion = config_get_int(RecordGlobal::instance()->GlobalConfig(), "General",
					      "LastRCVersion");
#else
	uint32_t lastVersion =
		config_get_int(RecordGlobal::instance()->GlobalConfig(), "General", "LastVersion");
#endif
...
#endif
		config_set_int(RecordGlobal::instance()->GlobalConfig(), "General",
			       "InfoIncrement", -1);
	} else {
		current_version_increment = config_get_int(
			RecordGlobal::instance()->GlobalConfig(), "General", "InfoIncrement");
	}
	...
	config_set_int(RecordGlobal::instance()->GlobalConfig(), "General", "InfoIncrement",
		       info_increment);
...
OBSBasic::~OBSBasic()
{
	...
	config_set_int(RecordGlobal::instance()->GlobalConfig(), "General", "LastVersion",
		       LIBOBS_API_VER);
#if OBS_RELEASE_CANDIDATE > 0
	config_set_int(RecordGlobal::instance()->GlobalConfig(), "General", "LastRCVersion",
		       OBS_RELEASE_CANDIDATE_VER);
#endif

	bool alwaysOnTop = IsAlwaysOnTop(this);

	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "PreviewEnabled",
			previewEnabled);
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "AlwaysOnTop",
			alwaysOnTop);
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
			"SceneDuplicationMode", sceneDuplicationMode);
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "SwapScenesMode",
			swapScenesMode);
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
...
#if 0
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "DocksLocked",
			ui->lockUI->isChecked());
#endif
	config_save_safe(RecordGlobal::instance()->GlobalConfig(), "tmp", nullptr);
	...
	obs_hotkey_register_source(
		...
		[](void *data, obs_hotkey_id, obs_hotkey_t *, bool pressed) {
			OBSBasic *main = reinterpret_cast<OBSBasic *>(
				RecordGlobal::instance()->GetRecordDialog());
...
void OBSBasic::MixerRenameSource()
{
	...
	blog(LOG_ERROR, "Source '%s' renamed ", prevName);
	for (;;) {
		string name;
#if 0
...
{
	if (!config_get_bool(RecordGlobal::instance()->GlobalConfig(), "General",
			     "EnableAutoUpdates"))
		return;

#ifdef UPDATE_SPARKLE
	init_sparkle_updater(config_get_bool(RecordGlobal::instance()->GlobalConfig(), "General",
					     "UpdateToUndeployed"));
#elif _WIN32
	long long lastUpdate = config_get_int(RecordGlobal::instance()->GlobalConfig(), "General",
					      "LastUpdateCheck");
	uint32_t lastVersion =
		config_get_int(RecordGlobal::instance()->GlobalConfig(), "General", "LastVersion");
...
int OBSBasic::ResetVideo()
{
	...
	ovi.graphics_module = RecordGlobal::instance()->GetRenderModule();
	ovi.base_width =
	...
	ovi.range = astrcmpi(colorRange, "Full") == 0 ? VIDEO_RANGE_FULL
						      : VIDEO_RANGE_PARTIAL;
	ovi.adapter =
		config_get_uint(RecordGlobal::instance()->GlobalConfig(), "Video", "AdapterIdx");
...
void OBSBasic::closeEvent(QCloseEvent *event)
{
	...
	if (isVisible())
		config_set_string(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
				  "geometry",
	...
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "DockState",
	...
	// RecordGlobal::instance()->quit(); // storm
}
...
void OBSBasic::on_actionAdvAudioProperties_triggered()
{
	...
	bool iconsVisible = config_get_bool(RecordGlobal::instance()->GlobalConfig(),
...
void OBSBasic::on_scenes_itemDoubleClicked(QListWidgetItem *witem)
{
	...
	if (IsPreviewProgramMode()) {
		bool doubleClickSwitch =
			config_get_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow",
...
{
	MyAddSourceData *data = (MyAddSourceData *)_data;
...
void OBSBasic::UploadLog(const char *subdir, const char *file)
{
	...
	stringstream ss;
	ss << "OBS " << RecordGlobal::instance()->GetVersionString() << " log file uploaded at "
...
{
	// UploadLog("obs-studio/logs", RecordGlobal::instance()->GetCurrentLog());
}

void OBSBasic::on_actionUploadLastLog_triggered()
{
	// UploadLog("obs-studio/logs", RecordGlobal::instance()->GetLastLog());
}
...
void OBSBasic::on_actionViewCurrentLog_triggered()
{
	...
	const char *log = RecordGlobal::instance()->GetCurrentLog();
...
{
	UploadLog("obs-studio/crashes", RecordGlobal::instance()->GetLastCrashLog());
}
...
{
	const char *priority = config_get_string(RecordGlobal::instance()->GlobalConfig(),
...
inline void OBSBasic::OnActivate()
{
	...
	if (ui->profileMenu->isEnabled()) {
		...
		RecordGlobal::instance()->IncrementSleepInhibition();
...
inline void OBSBasic::OnDeactivate()
{
	...
	if (!outputHandler->Active() && !ui->profileMenu->isEnabled()) {
		...
		RecordGlobal::instance()->DecrementSleepInhibition();
...
void OBSBasic::ShowReplayBufferPauseWarning()
{
#if 0
	auto msgBox = []() {
		QMessageBox msgbox(RecordGlobal::instance()->GetRecordDialog()); // storm
		//QMessageBox msgbox();
	...
	bool warned = config_get_bool(RecordGlobal::instance()->GlobalConfig(), "General",
...
void OBSBasic::UpdateTitleBar()
{
	...
	const char *profile =
		config_get_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "Profile");
	const char *sceneCollection = config_get_string(
		RecordGlobal::instance()->GlobalConfig(), "Basic", "SceneCollection");
	...
	name << RecordGlobal::instance()->GetVersionString();
	if (RecordGlobal::instance()->IsPortableMode())
		name << " - Portable Mode";
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	const char *profile =
		config_get_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "ProfileDir");
	int ret;
...
void OBSBasic::on_toggleStatusBar_toggled(bool visible)
{
	...
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "ShowStatusBar",
			visible);
}
...
#endif
	config_set_bool(RecordGlobal::instance()->GlobalConfig(), "BasicWindow", "ShowSourceIcons",
			visible);
}
...
{
	return reinterpret_cast<OBSBasic *>(RecordGlobal::instance()->GetRecordDialog());
	//return nullptr;
}
#if 0
...
void OBSBasic::SetControlProperties() // storm
{
	...
	int cx = 0;//// (int)config_get_int(RecordGlobal::instance()->GlobalConfig(), "PropertiesWindow", "cx");
	int cy = 0;//(int)config_get_int(RecordGlobal::instance()->GlobalConfig(), "PropertiesWindow", "cy");
...
void OBSBasic::on_themeComboBox_currentIndexChanged(int idx)
{
	...
	//if (WidgetChanged(ui->themeComboBox)) 
    {
		...
		// RecordGlobal::instance()->SetTheme(themeData.toUtf8().constData()); //storm
	}
}
...
{
	const char *currentLang = RecordGlobal::instance()->GetLocale();
...
void OBSBasic::LoadThemeList()
{
	...
	savedTheme = string(RecordGlobal::instance()->GetTheme());
	...
	std::string themeName = RecordGlobal::instance()->GetTheme();
