			  ...
			  const char *init_text, bool rename)
{
	...
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "Profile",
			  newName.c_str());
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "ProfileDir",
			  newDir.c_str());
...
void OBSBasic::on_actionRemoveProfile_triggered()
{
	...
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "Profile",
			  newName.c_str());
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "ProfileDir", newDir);
#if 0
	Auth::Save();
	auth.reset();
	DeleteCookies();
...
void OBSBasic::on_actionExportProfile_triggered()
{
	...
	QString currentProfile = QString::fromUtf8(config_get_string(
		RecordGlobal::instance()->GlobalConfig(), "Basic", "ProfileDir"));
...
void OBSBasic::ChangeProfile()
{
	...
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "Profile", newName);
	config_set_string(RecordGlobal::instance()->GlobalConfig(), "Basic", "ProfileDir", newDir);
#if 0
	Auth::Save();
	auth.reset();
