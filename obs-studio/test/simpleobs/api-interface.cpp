...
struct OBSStudioAPI : obs_frontend_callbacks {
	...
	{
		const char *name = config_get_string(RecordGlobal::instance()->GlobalConfig(),
						     "Basic", "Profile");
		return bstrdup(name);
	}
	...
	{
		return nullptr;//(void *)main->AddDockWidget((QDockWidget *)dock);
	}
	...
	{
		RecordGlobal::instance()->PushUITranslation(translate);
	}

	void obs_frontend_pop_ui_translation(void) override
	{
		RecordGlobal::instance()->PopUITranslation();
	}
