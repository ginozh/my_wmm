				    ...
				    QFormLayout *layout)
{
	...
	if (!obs_property_visible(property))
		return;
    blog(LOG_INFO, "property name: %s", name); // storm
...
class EditableItemDialog : public QDialog {
	...
	void BrowseClicked()
	{
		...
		QString path = QFileDialog::getOpenFileName(
			Q_NULLPTR/*App()->GetMainWindow()*/, QTStr("Browse"), curPath, //tmp storm
			filter);
		if (path.isEmpty())
			return;
...
void WidgetInfo::EditListAddFiles()
{
	...
	QStringList files = QFileDialog::getOpenFileNames(
		Q_NULLPTR/*App()->GetMainWindow()*/, title, QT_UTF8(default_path),
		QT_UTF8(filter));
...
void WidgetInfo::EditListEdit()
{
	...
	if (type == OBS_EDITABLE_LIST_TYPE_FILES) {
		...
		if (pathDir.exists())
			path = QFileDialog::getExistingDirectory(
				Q_NULLPTR/*App()->GetMainWindow()*/, QTStr("Browse"),
				item->text(),
				QFileDialog::ShowDirsOnly |
					QFileDialog::DontResolveSymlinks);
		else
