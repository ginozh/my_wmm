...
static void OBSStreamStopping(void *data, calldata_t *params)
{
	...
	int sec = (int)obs_output_get_active_delay(obj);
	if (sec == 0){
		 QMetaObject::invokeMethod(output->main, "StreamStopping"); // tmp storm
    } else{
		QMetaObject::invokeMethod(output->main, "StreamDelayStopping",
					  Q_ARG(int, sec)); // tmp storm
    }
}
...
static void OBSStartStreaming(void *data, calldata_t *params)
{
	...
	QMetaObject::invokeMethod(output->main, "StreamingStart"); //tmp storm
...
static void OBSStopStreaming(void *data, calldata_t *params)
{
	...
	QMetaObject::invokeMethod(output->main, "StreamingStop",
				  Q_ARG(int, code),
				  Q_ARG(QString, arg_last_error)); //storm
}
...
struct SimpleOutput : BasicOutputHandler {
	...
	SimpleOutput(RecordDialog *main_);
...
{
	const char *quality = "Stream"; // window-basic-main.cpp "RecQuality"
		// config_get_string(main->Config(), "SimpleOutput", "RecQuality");
	const char *encoder = SIMPLE_ENCODER_X264; // window-basic-main.cpp "RecEncoder"
...
SimpleOutput::SimpleOutput(RecordDialog *main_) : BasicOutputHandler(main_)
{
	const char *encoder = SIMPLE_ENCODER_X264; // 
        //config_get_string(main->Config(), "SimpleOutput", "StreamEncoder");
...
{
	int bitrate = 160;
        // (int)config_get_uint(main->Config(), "SimpleOutput", "ABitrate");
...
void SimpleOutput::Update()
{
	...
	bool enforceBitrate = true;
        // config_get_bool(main->Config(), "SimpleOutput", "EnforceBitrate");
	const char *custom = "";
        // config_get_string(main->Config(), "SimpleOutput", "x264Settings"); // tmp storm
	const char *encoder = SIMPLE_ENCODER_X264;
        // config_get_string(main->Config(), "SimpleOutput", "StreamEncoder");
	...
	preset = "veryfast";//config_get_string(main->Config(), "SimpleOutput", presetType);
...
int SimpleOutput::CalcCRF(int crf)
{
	...
	uint32_t cx = 1280;//primaryScreen->size().width();  // tmp storm
	uint32_t cy = 720;//primaryScreen->size().height();
...
{
	const char *path = "."; // "/Users/user/Movies"
	...
	const char *mux = "";
        // config_get_string(main->Config(), "SimpleOutput", "MuxerCustom");
	bool noSpace = false;
        // config_get_bool(main->Config(), "SimpleOutput", "FileNameWithoutSpace");
	...
	const char *rbPrefix = "Replay";
        // config_get_string(main->Config(), "SimpleOutput", "RecRBPrefix");
	const char *rbSuffix = "";
        // config_get_string(main->Config(), "SimpleOutput", "RecRBSuffix");
	int rbTime = 20;//
...
struct AdvancedOutput : BasicOutputHandler {
	...
	AdvancedOutput(RecordDialog *main_);
...
static OBSData GetDataFromJsonFile(const char *jsonFile)
{
	...
	int ret = 0;// GetProfilePath(fullPath, sizeof(fullPath), jsonFile); //tmp storm
	if (ret > 0) {
...
AdvancedOutput::AdvancedOutput(RecordDialog *main_) : BasicOutputHandler(main_)
{
#if 0
	const char *recType =
...
BasicOutputHandler *CreateSimpleOutputHandler(RecordDialog *main)
{
	return new SimpleOutput(main);
}

BasicOutputHandler *CreateAdvancedOutputHandler(RecordDialog *main)
{
