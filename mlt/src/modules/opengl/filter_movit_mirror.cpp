...
static int get_image( mlt_frame frame, uint8_t **image, mlt_image_format *format, int *width, int *height, int writable )
{
    ...
    fprintf( stdout, "filter_movit_mirror::get_image\n");
...
{
    fprintf( stdout, "filter_movit_mirror::process\n");
...
mlt_filter filter_movit_mirror_init( mlt_profile profile, mlt_service_type type, const char *id, char *arg )
{
	...
	if ( glsl && ( filter = mlt_filter_new() ) ) {
        fprintf( stdout, "filter_movit_mirror::filter_movit_mirror_init 2\n");
		mlt_properties properties = MLT_FILTER_PROPERTIES( filter );
		glsl->add_ref( properties );
		filter->process = process;
	}
    fprintf( stdout, "filter_movit_mirror::filter_movit_mirror_init glsl: %ld filter: %ld\n", (long)glsl, (long)filter);
	return filter;
}
