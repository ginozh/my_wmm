	...
	int64_t req_position, int preseek )
{
	...
	if ( self->video_seekable && ( position != self->video_expected || self->last_position < 0 ) )
	{
		...
		else if ( position < self->video_expected || position - self->video_expected >= seek_threshold || self->last_position < 0 )
		{
			...
			if ( timestamp < 0 )
				timestamp = 0;
			mlt_log_warning( MLT_PRODUCER_SERVICE(producer), "seeking timestamp %"PRId64" position " 
                    MLT_POSITION_FMT " req_position %"PRId64" preseek %d first_pts %"PRId64
                    " start_time %"PRId64" AV_NOPTS_VALUE %"PRId64" source_fps %f self->video_time_base %d/%d expected "
                    MLT_POSITION_FMT" last_pos %"PRId64"\n"
                , timestamp, position, req_position, preseek, self->first_pts, context->start_time, AV_NOPTS_VALUE
                , source_fps, self->video_time_base.num, self->video_time_base.den
                , self->video_expected, self->last_position );
		...
		while( ret >= 0 && !got_picture )
		{
			...
			if ( self->pkt.stream_index == self->video_index )
			{
				...
				mlt_log_warning( MLT_PRODUCER_SERVICE(producer),
					"V pkt.pts %"PRId64" pkt.dts %"PRId64" req_pos %"PRId64" cur_pos %"PRId64" last_position %"PRId64" pkt_pos %"PRId64" pts %"PRId64" \n",
					self->pkt.pts, self->pkt.dts, req_position, self->current_position, self->last_position, int_position, pts);
				...
				if ( must_decode  || int_position >= req_position || !self->pkt.data )
				{
					...
					mlt_log_warning( MLT_PRODUCER_SERVICE(producer), "decoded packet with size %d => %d\n", self->pkt.size, ret );
				...
				mlt_log_warning( MLT_PRODUCER_SERVICE(producer), " got_pic %d key %d ret %d pkt_pos %"PRId64"\n",
