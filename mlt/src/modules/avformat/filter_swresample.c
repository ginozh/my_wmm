...
static int configure_swr_context( mlt_filter filter )
{
	...
	else
	{
		...
		double* matrix= av_mallocz_array( pdata->in_channels * pdata->out_channels, sizeof(double) ); //storm
	...
	pdata->in_buffers = av_mallocz_array( pdata->in_channels, sizeof(uint8_t*) ); //storm
	av_freep( &pdata->out_buffers );
	pdata->out_buffers = av_mallocz_array( pdata->out_channels, sizeof(uint8_t*) );//storm
