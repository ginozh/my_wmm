...
inline void FFmpegReader::fill_rectangle(SDL_Surface *screen,
...
void FFmpegReader::blend_subrect(uint8_t *dst[4], uint16_t dst_linesize[4],
...
void FFmpegReader::free_subpicture(SubPicture *sp)
{
    avsubtitle_free(&sp->sub);
}

void FFmpegReader::video_image_display(PlayerState *is)
{
    VideoPicture *vp;
    SubPicture *sp;
    float aspect_ratio;
...
inline int FFmpegReader::compute_mod(int a, int b)
{
    a = a % b;
    if (a >= 0)
        return a;
...
void FFmpegReader::video_audio_display(PlayerState *s)
{
...
int FFmpegReader::video_open(PlayerState *is)
{
...
void FFmpegReader::video_display(PlayerState *is)
{
    if (!screen)
        video_open(player);
...
/* seek in the stream */
static void stream_seek(PlayerState *is, int64_t pos, int64_t rel, int seek_by_bytes) //kuli update storm
{
    if (!is->seek_req) {
        is->seek_pos = pos;
        is->seek_rel = rel;
...
static void stream_pause(PlayerState *is) //kuli update storm 
{
    if (is->paused) {
...
QImage FFmpegReader::GetFrame(long int request_frame)
{
    ...
    QImage image;
    while(1){
        image=GetOneFrame(request_frame);
        if(!image.isNull())
        {
            break;
        }
        if(mis.eof && mis.videoq.size==0)
        {
            av_log(NULL, AV_LOG_INFO, "video end\n");
            SDL_PauseAudio(0);
            break;
        }
    }
...
QImage FFmpegReader::GetOneFrame(long int request_frame)
{
    QImage image;
    PlayerState *is = &mis;
    VideoPicture *vp;
...
void FFmpegReader::video_refresh_timer(void *opaque)
{
    PlayerState *is = (PlayerState*)opaque;
    VideoPicture *vp;
...
void FFmpegReader::player_close(PlayerState *is) // kuli update storm : stream_close
{
    VideoPicture *vp;
    int i;
...
void FFmpegReader::do_exit(void) //kuli update storm
{
    if (player) {
        player_close(player);
        player = NULL;
...
void FFmpegReader::alloc_picture(void *opaque)
{
    PlayerState *is = (PlayerState*)opaque;
    VideoPicture *vp;
...
static int queue_picture(PlayerState *is, AVFrame *src_frame, double pts, int64_t pos) //kuli update storm: add filter for frame
{
    VideoPicture *vp;
...
static int queue_picture(PlayerState *is, AVFrame *src_frame, double pts, int64_t pos) //kuli update storm: add filter for frame
{
    ...
    sws_scale(img_convert_ctx,
            ...
            src_frame->linesize, 0, pCodecCtx->height, pFrameRGB->data,
            pFrameRGB->linesize);
    //把这个RGB数据 用QImage加载
    QImage tmpImg((uchar *)out_buffer_rgb,pCodecCtx->width,pCodecCtx->height,QImage::Format_RGB32);
    vp->image = tmpImg.copy();     //把图像复制一份 传递给界面显示
...
static int configure_video_filters(AVFilterGraph *graph, PlayerState *is, const char *vfilters)
{
    ...
    snprintf(sws_flags_str, sizeof(sws_flags_str), "flags=%ld", sws_flags);
...
int VideoThread::initial(PlayerState *is)
{
    int ret=0;
    mis=is;
    ...
    if ((ret = configure_video_filters(mgraph, is, is->vfilters)) < 0)
    {
        //uncomplete error
        return ret;
    }
...
void VideoThread::run() // from video_thread
{
    AVPacket pkt = { 0 };
    PlayerState *is = mis;
    ...
    double pts;
    int ret;
    static int idx=0; // storm
    AVFilterGraph *graph = mgraph;
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ret = av_buffersink_get_frame(filt_out, frame); // kuli update storm
            if (ret < 0) {
                ret = 0;
                break;
            }
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    int audio_size, len1;
    double pts;
    av_log(NULL, AV_LOG_INFO, "sdl_audio_callback len: %d\n", len);

    is->audio_callback_time = av_gettime_relative(); //storm
    ...
    while (len > 0) {
        if (is->audio_buf_index >= is->audio_buf_size) {
            ...
            if (audio_size < 0) {
                ...
                av_log(NULL, AV_LOG_INFO, "sdl_audio_callback audio_size: %d pts: %lf\n"
                        , audio_size, pts);
        ...
        av_log(NULL, AV_LOG_INFO, "sdl_audio_callback len: %d len1: %d audio_buf_size: %d audio_size: %d\n"
                , len, len1, is->audio_buf_size, audio_size);
        if (len1 > len)
            len1 = len;
...
AVCodec *FFmpegReader::find_codec_or_die(const char *name, enum AVMediaType type)
{
...
AVCodec *FFmpegReader::choose_decoder(PlayerState *is, AVFormatContext *ic, AVStream *st)
{
...
{
    PlayerState *global_video_state=(PlayerState*)ctx;
...
int FFmpegReader::stream_setup(PlayerState *is)
{
    ...
    ic->interrupt_callback.opaque = is->global_video_state;
...
int DecodeThread::initial(PlayerState *is)
{
    mis=is;
    return 0;
}
/* this thread gets the stream from the disk or the network */
void DecodeThread::run() //from decode_thread; kuli update storm
{
    PlayerState *is        = mis;
    AVPacket pkt1, *pkt   = &pkt1;
    AVFormatContext *ic   = is->ic;
        ...
        if (is->eof) {
            ...
            if (is->audioq.size + is->videoq.size + is->subtitleq.size == 0) {
                if (is->loop != 1 && (!is->loop || --is->loop)) {
                    //stream_seek(player, is->start_time != AV_NOPTS_VALUE ? is->start_time : 0, 0, 0);
...
/* this thread gets the stream from the disk or the network */
static int decode_thread(void *arg) // kuli update storm
{
    PlayerState *is        = (PlayerState*)arg;
    AVPacket pkt1, *pkt   = &pkt1;
    AVFormatContext *ic   = is->ic;
...
int FFmpegReader::stream_open(PlayerState *is,
        const char *filename, AVInputFormat *iformat)
{
    ...
    if (!is->parse_tid)
        return -1;
#endif
    mDecodeThread.initial(is);
    return 0;
}

void FFmpegReader::stream_cycle_channel(PlayerState *is, int codec_type)
{
    AVFormatContext *ic = is->ic;
...
void FFmpegReader::toggle_full_screen(void)
{
#if defined(__APPLE__) && SDL_VERSION_ATLEAST(1, 2, 14)
...
void FFmpegReader::toggle_pause(void)
{
    if (player)
        stream_pause(player);
    player->step = 0;
...
void FFmpegReader::step_to_next_frame(void)
{
    if (player) {
...
void FFmpegReader::toggle_audio_display(void)
{
    if (player) {
...
void FFmpegReader::seek_chapter(PlayerState *is, int incr)
{
...
void FFmpegReader::event_loop(void)
{
    SDL_Event event;
...
do_seek:
            ...
            case FF_ALLOC_EVENT:
                ...
                if (player->out_video_filter) {
                    video_open((PlayerState*)event.user.data1);
...
#endif 
#if 0
int FFmpegReader::opt_frame_size(void *optctx, const char *opt, const char *arg)
{
...
int FFmpegReader::opt_width(void *optctx, const char *opt, const char *arg)
{
    screen_width = parse_number_or_die(opt, arg, OPT_INT64, 1, INT_MAX);
    return 0;
}

int FFmpegReader::opt_height(void *optctx, const char *opt, const char *arg)
{
    screen_height = parse_number_or_die(opt, arg, OPT_INT64, 1, INT_MAX);
    return 0;
}

int FFmpegReader::opt_format(void *optctx, const char *opt, const char *arg)
{
...
int FFmpegReader::opt_frame_pix_fmt(void *optctx, const char *opt, const char *arg)
{
    av_log(NULL, AV_LOG_ERROR,
            "Option '%s' has been removed, use private format options instead\n", opt);
    return AVERROR(EINVAL);
...
int FFmpegReader::opt_sync(void *optctx, const char *opt, const char *arg)
{
    if (!strcmp(arg, "audio"))
...
int FFmpegReader::opt_seek(void *optctx, const char *opt, const char *arg)
{
    start_time = parse_time_or_die(opt, arg, 1);
    return 0;
}

int FFmpegReader::opt_duration(void *optctx, const char *opt, const char *arg)
{
...
void FFmpegReader::opt_input_file(void *optctx, const char *filename)
{
    if (input_filename) {
...
FFmpegReader::FFmpegReader(){    //开始时播放状态为stop
    mPlayerState = Stop;
    circling = false;
    memset(&mis, 0, sizeof(mis));
    mis.audio_callback_time = 0;
    mis.loop = 1;
    mis.infinite_buffer = 0;
    mis.autorotate = 1;
    mis.noautoexit = 1;
    mis.decoder_reorder_pts = -1;
    mis.start_time = AV_NOPTS_VALUE;
    mis.duration = AV_NOPTS_VALUE;
    mis.step = 0;
    mis.global_video_state = NULL;
...
bool FFmpegReader::pause(){           //暂停
    toggle_pause();
#if 0
    //mVideoState.isPause = true;
    mVideoState.paused = 1;
...
bool FFmpegReader::stop(bool isWait){    //停止
    SDL_Delay(10);
...
void FFmpegReader::seek(int64_t pos){   //跳转
...
double FFmpegReader::getCurrentTime(){   //当前时间
#if 0
    if(mVideoState.audioID != 0){
...
int64_t FFmpegReader::getTotalTime(){   //总时间
    return mVideoState.ic->duration;
}

void FFmpegReader::disPlayVideo(QImage img){    //发送信号，将视频解析出的QImage发过去
    emit sig_GetOneFrame(img);
}

static char file_path[1280] = {0};
bool FFmpegReader::setFileName(QString path)
{     //读取视频文件路径
    int flags;
    if (mPlayerState != Stop){
        return false;
    ...
    if (display_disable) {
        video_disable = 0; //storm  配合display_disable=1 来使用，否则没有图像
    }
...
#if 0
void FFmpegReader::run(){             //读取视频，寻找流信息，打开解码器
    SDL_PauseAudio(0);
    event_loop();
}
#endif
