...
inline void VideoThread::fill_rectangle(SDL_Surface *screen,
...
void VideoThread::blend_subrect(uint8_t *dst[4], uint16_t dst_linesize[4],
...
void VideoThread::free_subpicture(SubPicture *sp)
{
    avsubtitle_free(&sp->sub);
}

void VideoThread::video_image_display(PlayerState *is)
{
    VideoPicture *vp;
    SubPicture *sp;
    float aspect_ratio;
...
inline int VideoThread::compute_mod(int a, int b)
{
    a = a % b;
    if (a >= 0)
        return a;
...
void VideoThread::video_audio_display(PlayerState *s)
{
...
int VideoThread::video_open(PlayerState *is)
{
...
void VideoThread::video_display(PlayerState *is)
{
    if (!screen)
        video_open(player);
...
void VideoThread::video_refresh_timer(void *opaque)
{
    PlayerState *is = (PlayerState*)opaque;
    VideoPicture *vp;
...
void VideoThread::player_close(PlayerState *is) // kuli update storm : stream_close
{
    VideoPicture *vp;
    int i;
...
void VideoThread::do_exit(void) //kuli update storm
{
    if (player) {
        player_close(player);
        player = NULL;
...
void VideoThread::alloc_picture(void *opaque)
{
    PlayerState *is = (PlayerState*)opaque;
    VideoPicture *vp;
...
AVCodec *VideoThread::find_codec_or_die(const char *name, enum AVMediaType type)
{
...
AVCodec *VideoThread::choose_decoder(PlayerState *is, AVFormatContext *ic, AVStream *st)
{
...
int VideoThread::stream_component_open(PlayerState *is, int stream_index)
{
...
int VideoThread::stream_setup(PlayerState *is)
{
...
int VideoThread::stream_open(PlayerState *is,
...
void VideoThread::stream_cycle_channel(PlayerState *is, int codec_type)
{
    AVFormatContext *ic = is->ic;
...
void VideoThread::toggle_full_screen(void)
{
#if defined(__APPLE__) && SDL_VERSION_ATLEAST(1, 2, 14)
...
void VideoThread::toggle_pause(void)
{
    if (player)
        stream_pause(player);
    player->step = 0;
...
void VideoThread::step_to_next_frame(void)
{
    if (player) {
...
void VideoThread::toggle_audio_display(void)
{
    if (player) {
...
void VideoThread::seek_chapter(PlayerState *is, int incr)
{
...
void VideoThread::event_loop(void)
{
    SDL_Event event;
...
#if 0
int VideoThread::opt_frame_size(void *optctx, const char *opt, const char *arg)
{
...
int VideoThread::opt_width(void *optctx, const char *opt, const char *arg)
{
    screen_width = parse_number_or_die(opt, arg, OPT_INT64, 1, INT_MAX);
    return 0;
}

int VideoThread::opt_height(void *optctx, const char *opt, const char *arg)
{
    screen_height = parse_number_or_die(opt, arg, OPT_INT64, 1, INT_MAX);
    return 0;
}

int VideoThread::opt_format(void *optctx, const char *opt, const char *arg)
{
...
int VideoThread::opt_frame_pix_fmt(void *optctx, const char *opt, const char *arg)
{
    av_log(NULL, AV_LOG_ERROR,
            "Option '%s' has been removed, use private format options instead\n", opt);
    return AVERROR(EINVAL);
...
int VideoThread::opt_sync(void *optctx, const char *opt, const char *arg)
{
    if (!strcmp(arg, "audio"))
...
int VideoThread::opt_seek(void *optctx, const char *opt, const char *arg)
{
    start_time = parse_time_or_die(opt, arg, 1);
    return 0;
}

int VideoThread::opt_duration(void *optctx, const char *opt, const char *arg)
{
...
bool VideoThread::setFileName(QString path)
{     //读取视频文件路径
    int flags;
    if (mPlayerState != Stop){
        return false;
