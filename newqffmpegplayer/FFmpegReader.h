...
class DecodeThread : public QThread
{
    ...
    public:
        DecodeThread(){}
        ~DecodeThread(){}
        int initial(PlayerState *is); 
    protected:
        void run();                    //QThread的run函数
    private:
        PlayerState* mis=NULL;
};
...
signals:
        ...
        void sig_StateChanged(FFmpegReader::PlayState state);  //状态改变信号
    ...
    private:
        ...
        int wanted_stream[AVMEDIA_TYPE_NB];
        int seek_by_bytes = -1;
        int display_disable=1;
        int show_status = 1;
        int av_sync_type = AV_SYNC_AUDIO_MASTER; //AV_SYNC_EXTERNAL_CLOCK;//AV_SYNC_VIDEO_MASTER; //AV_SYNC_AUDIO_MASTER;
        //int64_t start_time = AV_NOPTS_VALUE;
        //int64_t duration = AV_NOPTS_VALUE;
        //int step = 0;
