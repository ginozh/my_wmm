...
//	vcodec_ctx = video_stream->codec;
    ...
    video_compression_type = COMPRESSION_TYPE_CBR; //storm
...
void ExportThread::run() {
    ...
    int64_t wasteTimeSt,wasteTimeGF, wasteTimeGI, wasteTimeSI, wasteTimeGAF, wasteTimeWF;//storm
	...
	while (sequence->playhead <= end_frame && continueEncode) {
		...
		wasteTimeGF = QDateTime::currentMSecsSinceEpoch();
		...
		if (video_enabled) {
            ...
            qDebug()<<"ExportThread::run pts: "<<sws_frame->pts<<" number: "<<(sequence->playhead-start_frame)
                <<" frame_rate: "<< sequence->frame_rate<<" timecode_secs: "<<timecode_secs
                <<" time_base: "<<video_stream->time_base.num<<video_stream->time_base.den;
            wasteTimeSI = QDateTime::currentMSecsSinceEpoch();
			// send to encoder
			if (!encode(fmt_ctx, vcodec_ctx, sws_frame, &video_pkt, video_stream, false)) continueEncode = false;
            wasteTimeWF = QDateTime::currentMSecsSinceEpoch();
		}
		if (audio_enabled) {
			...
			while (continueEncode && file_audio_samples <= (timecode_secs*audio_sampling_rate)) {
                ...
                qInfo()<<"ExportThread::run in frame. "
                    <<" channels: "<<audio_frame->channels<<" channel_layout: "<<audio_frame->channel_layout
                    <<" sample_rate: "<<audio_frame->sample_rate<<" format: "<<audio_frame->format
                    <<" nb_samples: "<<audio_frame->nb_samples
                    <<" out frame. "
                    <<" channels: "<<swr_frame->channels<<" channel_layout: "<<swr_frame->channel_layout
                    <<" sample_rate: "<<swr_frame->sample_rate<<" format: "<<swr_frame->format
                    <<" nb_samples: "<<swr_frame->nb_samples<<" file_audio_samples: "<<file_audio_samples;
				// send to encoder
		...
		wasteTimeGAF = QDateTime::currentMSecsSinceEpoch();
...
//        qInfo() << "Encoded frame" << sequence->playhead << "- took" << frame_time << "ms (avg:" << avg_time << "ms, remaining:" << remaining_frames << ", ETA:" << eta << ")";
        qInfo()<< "ExportThread::run video_position: "<<sequence->playhead<<" getframe: " << (wasteTimeGF-wasteTimeSt)
            <<" scaleimage"<<(wasteTimeSI-wasteTimeGF)
            <<" getaudio: "<<(wasteTimeGAF-wasteTimeWF) <<" writeframe: "<<(wasteTimeWF-wasteTimeSI);//storm
