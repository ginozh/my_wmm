...
void get_clip_frame(Clip* c, long playhead, bool& texture_failed) {
	if (c->finished_opening) {
		...
		if (target_frame != nullptr) {
            ...
            //QImage tmpImg((uchar *)(using_db_1 ? data_buffer_1 : data_buffer_2),target_frame->width,target_frame->height,QImage::Format_RGBA8888); static int idx=0;tmpImg.save(QString("%1.jpg").arg(++idx));//storm
			if (data_buffer_1 != target_frame->data[0]) {
...
int retrieve_next_frame(Clip* c, AVFrame* f) {
	...
	while ((receive_ret = avcodec_receive_frame(c->codecCtx, f)) == AVERROR(EAGAIN)) {
		...
		do {
            ...
            if(read_ret >= 0) {
                //qDebug()<<"retrieve_next_frame c->pkt->stream_index: "<<c->pkt->stream_index <<" media_stream: "<<c->media_stream;
            }
