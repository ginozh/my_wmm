...
void TransformEffect::process_coords(double timecode, GLTextureCoords& coords, int) {
	...
	glScalef(sx, sy, 1);
    qDebug()<<"TransformEffect::process_coords anchor_x_offset: "<<anchor_x_offset<<" anchor_y_offset: "<<anchor_y_offset<<" position "<<(position_x->get_double_value(timecode)-(parent_clip->sequence->width/2))<<(position_y->get_double_value(timecode)-(parent_clip->sequence->height/2))<<" rotation: "<<rotation->get_double_value(timecode)<<" scale: "<<sx<<sy<<" blend mode: "<<blend_mode_box->get_combo_data(timecode).toInt();
	// blend mode
