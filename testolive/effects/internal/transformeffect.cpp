...
TransformEffect::TransformEffect(Clipt* c, const EffectMeta* em) : Effect(c, em) {
	enable_coords = true;
...
void TransformEffect::refresh() {
    ...
    double new_default_pos_x = width/2;
    double new_default_pos_y = height/2;
    ...
    double x_percent_multipler = 200.0 / width;
    double y_percent_multipler = 200.0 / height;
...
void TransformEffect::process_coords(double timecode, GLTextureCoords& coords, int) {
	...
	// position
	glTranslatef(position_x->get_double_value(timecode)-(width/2), position_y->get_double_value(timecode)-(height/2), 0);
	...
	glScalef(sx, sy, 1);
    qDebug()<<"TransformEffect::process_coords anchor_x_offset: "<<anchor_x_offset<<" anchor_y_offset: "<<anchor_y_offset<<" position "<<(position_x->get_double_value(timecode)-(width/2))<<(position_y->get_double_value(timecode)-(height/2))<<" rotation: "<<rotation->get_double_value(timecode)<<" scale: "<<sx<<sy<<" blend mode: "<<blend_mode_box->get_combo_data(timecode).toInt();
	// blend mode
