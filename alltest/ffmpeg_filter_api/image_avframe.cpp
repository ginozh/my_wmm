...
int image_to_avframe(const char *src_filename, int dstWidth, int dstHeight, int dstFormat, AVFrame * &out)
{
...
#if 0
    if(frame->format!=dstFormat || frame->width!=dstWidth || frame->height!=dstHeight)
    {
    }
#endif
    out=convertFormat(frame, dstWidth, dstHeight, dstFormat);
    //scale
    //return 0;
fail:
    image_avframe_close(fmt_ctx, video_dec_ctx);
