...
static int transcode(void)
{
    ...
    for (i = 0; i < nb_output_files; i++) {
        os = output_files[i]->ctx; //if(os->pb && os->pb->opaque){struct buffer_data *bd = (struct buffer_data *)os->pb->opaque;} //storm
...
//     }
    ...
    av_log(NULL, AV_LOG_INFO, "%"PRIu64" frames successfully decoded, %"PRIu64" decoding errors\n",
...
// ./ffmpeg -y -f concat -i "buffer:text/txt;testconcat.txt"  -c copy jpg/mm.mp4
// cat testconcat.txt
// file buffer:video/mp4;jpg/512.1.mp4
// file buffer:video/mp4;jpg/512.2.mp4
static int test_concat(int argc, char **argv)
{
    struct to_buffer sinbuffer, sinbuffer1, sinbuffer2;
...
// ./ffmpeg -y -f concat -i "buffer:text/txt;testconcat.txt"  -c copy jpg/mm.mp4
    ...
    ret = av_file_map("jpg/512.1.mp4", &in_buffer1, &in_len1, 0, NULL);
    if (ret < 0)
        exit_program(1);
    sinbuffer1.ptr = in_buffer1;
    sinbuffer1.in_len = in_len1;
    sinbuffer1.out_len = NULL;
    ret = av_file_map("jpg/512.2.mp4", &in_buffer2, &in_len2, 0, NULL);
    if (ret < 0)
        exit_program(1);
    ...
    // file buffer:video/mp4;jpg/512.1.mp4
    // file buffer:video/mp4;jpg/512.2.mp4
    snprintf(in_buffer, len, "file buffer:video/mp4;nobase64,%zu\nfile buffer:video/mp4;nobase64,%zu\n", (size_t)&sinbuffer1, (size_t)&sinbuffer2);
    ...
    snprintf(argvv[i++], len, "jpg/mm.mp4");
    qt_ffmpeg(i, argvv);
...
{
    struct to_buffer sinbuffer, sinbuffer2, soutbuffer;
    int i=0;
    uint8_t* in_buffer;
    uint8_t* in_buffer2;
    size_t in_len;
...
#define IN_BUFFER_PTR
    ...
    snprintf(argvv[i++], len, "1");
    snprintf(argvv[i++], len, "-i");
    ret = av_file_map("jpg/img002.jpg", &in_buffer2, &in_len2, 0, NULL);
    if (ret < 0)
        exit_program(1);
    sinbuffer2.ptr = in_buffer2;
    sinbuffer2.in_len = in_len2;
