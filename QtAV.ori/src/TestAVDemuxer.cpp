...
class TestAVDemuxer::TestInterruptHandler : public AVIOInterruptCB
{
public:
    enum Action {
        Unknown = -1,
    ...
    TestInterruptHandler(TestAVDemuxer* demuxer, int timeout = 30000)
      : mStatus(0)
      , mTimeout(timeout)
      , mTimeoutAbort(true)
      , mEmitError(true)
    ...
    */
    static int handleTimeout(void* obj) {
        TestInterruptHandler* handler = static_cast<TestInterruptHandler*>(obj);
        if (!handler) {
            qWarning("TestInterruptHandler is null");
            return -1;
        }
...
private:
    ...
    //qint64 mLastTime;
    Action mAction;
    TestAVDemuxer *mpDemuxer;
    QElapsedTimer mTimer;
};
class TestAVDemuxer::TestPrivate
{
public:
    TestPrivate()
    ...
    bool setStream(TestAVDemuxer::StreamType st, int streamValue);
    ...
    TestAVDemuxer::TestInterruptHandler *interrupt_hanlder;
