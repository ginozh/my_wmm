#ifndef QTAV_AVREADER_H
#define QTAV_AVREADER_H
...
class Q_AV_EXPORT AVReader : public QObject
{
    Q_OBJECT
//avplayer
    ...
    Q_PROPERTY(SState state READ state WRITE setState NOTIFY stateChanged)
...
public:
    ...
    const Statistics& fstatistics() const;
    /*!
     * \brief installFilter
...
Q_SIGNALS:
    ...
    void stateChanged(QtAV::AVReader::SState state);
...
//end avplayer
public:
    explicit AVReader(QObject *parent = 0);
    ~AVReader();
    ///void setFile(const QString& path);
public Q_SLOTS:
    ///bool load();
...
public:
    ...
    bool applySubtitleStream(int n, AVReader *player);
    bool setupAudioThread(AVReader *player);
    bool setupVideoThread(AVReader *player);
    bool tryApplyDecoderPriority(AVReader *player);
    ...
    bool bloaded; // for current source
    ...
    AVReader::SState vstate;
    MediaEndAction end_action;
    QMutex load_mutex;
//FFmpegReader
public:
    //bool setFileName(const QString& path, bool scaleImage, int scaleFlags, bool bOpenFile, bool bVideoDisable, bool bAudioDisable, bool bWebmSpecifyDecode); //save QtImageReader
    //SetProperty
    //void Open();
    //void Close(bool bUseLock=true);
    //std::shared_ptr<Frame> GetFrame(int64_t request_frame, int mapdiff=0);
    //std::shared_ptr<Frame> GetAudioFrame(int64_t request_frame);
    VideoFrame GetFrame(int64_t request_frame);
};

} //namespace QtAV

#endif // QTAV_AVREADER_H
