...
void FrameReader::readMoreInternal()
{
    ...
    Packet pkt;static qreal pts=0;//storm
    while (!d->demuxer.atEnd()) {
        ...
        if (d->decoder->decode(pkt)) {
            ...
            qInfo()<<"FrameReader::readMoreInternal get frame "<< frame.timestamp()<< pkt.pts<< pkt.dts;//storm
            printf("FrameReader::readMoreInternal get frame frame_pts: %0.3f packet_pts: %0.3f packet_dts: %0.3f\n",  frame.timestamp(), pkt.pts, pkt.dts);fflush(0);
#if 0
            if(pts<pkt.pts)
            {
                pts=pkt.pts;
                ...
                continue;
            {QImage img=frame.toImage();static int idx=0;++idx;printf("FrameReader::readMoreInternal img idx: %d  isNull: %d\n",idx,img.isNull());/*if(idx==3 || idx==15)*/ img.save(QString("images%1.jpg").arg(idx));}// storm
#endif
            d->vframes.put(frame);
