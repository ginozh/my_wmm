...
{
public:
    QueueEmptyCall(AVDemuxMyThread* thread):
        mDemuxThread(thread)
    {}
    virtual void call() {
        if (!mDemuxThread)
...
AVDemuxMyThread::AVDemuxMyThread(QObject *parent) :
    QThread(parent)
  , paused(false)
  , user_paused(false)
  , end(false)
...
AVDemuxMyThread::AVDemuxMyThread(AVDemuxer *dmx, QObject *parent) :
    QThread(parent)
  , paused(false)
  , end(false)
  , m_buffering(false)
...
void AVDemuxMyThread::setDemuxer(AVDemuxer *dmx)
{
    demuxer = dmx;
}

void AVDemuxMyThread::setAudioDemuxer(AVDemuxer *demuxer)
{
...
void AVDemuxMyThread::setAVThread(AVThread*& pOld, AVThread *pNew)
{
    if (pOld == pNew)
        return;
    if (pOld) {
...
void AVDemuxMyThread::setAudioThread(AVThread *thread)
{
    setAVThread(audio_thread, thread);
}

void AVDemuxMyThread::setVideoThread(AVThread *thread)
{
    setAVThread(video_thread, thread);
}

AVThread* AVDemuxMyThread::videoThread()
{
    return video_thread;
}

AVThread* AVDemuxMyThread::audioThread()
{
    return audio_thread;
}

void AVDemuxMyThread::stepBackward()
{
    if (!video_thread)
        return;
...
void AVDemuxMyThread::seek(qint64 external_pos, qint64 pos, SeekType type)
{
...
void AVDemuxMyThread::seekInternal(qint64 pos, SeekType type, qint64 external_pos)
{
...
void AVDemuxMyThread::newSeekRequest(QRunnable *r)
{
...
void AVDemuxMyThread::processNextSeekTask()
{
...
void AVDemuxMyThread::pauseInternal(bool value)
{
    paused = value;
}

bool AVDemuxMyThread::isPaused() const
{
    return paused;
}

bool AVDemuxMyThread::isEnd() const
{
    return end;
}

bool AVDemuxMyThread::atEndOfMedia() const
{
    return demuxer->atEnd();
}

PacketBuffer* AVDemuxMyThread::buffer()
{
    return m_buffer;
}

void AVDemuxMyThread::updateBufferState()
{
    if (!m_buffer)
        return;
...
void AVDemuxMyThread::pause(bool p, bool wait)
{
    user_paused = p;
    if (paused == p)
        return;
...
void AVDemuxMyThread::setMediaEndAction(MediaEndAction value)
{
    end_action = value;
}

MediaEndAction AVDemuxMyThread::mediaEndAction() const
{
    return end_action;
}

void AVDemuxMyThread::stepForward()
{
    if (end)
        return;
...
void AVDemuxMyThread::seekOnPauseFinished()
{
...
void AVDemuxMyThread::frameDeliveredOnStepForward()
{
    AVThread *thread = video_thread ? video_thread : audio_thread;
    Q_ASSERT(thread);
    QMutexLocker locker(&next_frame_mutex);
...
void AVDemuxMyThread::eofDecodedOnStepForward()
{
    AVThread *thread = video_thread ? video_thread : audio_thread;
    Q_ASSERT(thread);
    QMutexLocker locker(&next_frame_mutex);
...
void AVDemuxMyThread::onAVThreadQuit()
{
#if 0
    AVThread* av[] = { audio_thread, video_thread};
    for (size_t i = 0; i < sizeof(av)/sizeof(av[0]); ++i) {
...
bool AVDemuxMyThread::waitForStarted(int msec)
{
...
#endif
bool AVDemuxMyThread::tryPause(unsigned long timeout)
{
    if (!paused)
        return false;
...
#if 0
void AVDemuxMyThread::run() //storm
{
    m_buffering = false;
    end = false;
    if (video_thread && !video_thread->isRunning())
